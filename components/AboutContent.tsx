...
import { Users, Shield, Cloud, HeartHandshake, Headphones, Lock, Server, Settings, Zap } from 'lucide-react'
import { supabase } from '@/lib/supabase'
import type { TeamMember, AboutValue, AboutContent as AboutContentType } from '@/lib/supabase'
...
const defaultValues = [
  { icon: Users, title: 'Client-Focused', description: 'Your success is our priority. We tailor every solution to fit your unique business needs.' },
  { icon: Shield, title: 'Security First', description: 'Protecting your data and business with enterprise-grade security measures.' },
  { icon: Cloud, title: 'Cloud Experts', description: 'Specializing in Microsoft 365 cloud solutions that scale with your business.' },
  { icon: HeartHandshake, title: 'Always Available', description: '24/7 support to ensure your business never stops running smoothly.' },
]

const defaultCta = {
  cta_title: 'Ready to Transform Your Business?',
  cta_subtitle: 'Let\'s discuss how our M365 solutions can help you work smarter, safer, and more efficiently.',
...
export default function AboutContent() {
  ...
  useEffect(() => {
    ...
    async function fetchSectionVisibility() {
      ...
      if (!error && data) {
        setShowTeamSection(data.is_visible)
      }
    }
    ...
    async function fetchValues() {
      ...
      if (!error && data && data.length > 0) {
        setValues(data.map(v => ({
          icon: iconMap[v.icon_name] || Users,
          title: v.title,
    ...
    async function fetchCta() {
      const { data, error } = await supabase
        .from('about_content')
        .select('cta_title, cta_subtitle, cta_button_text')
        .single()

      if (!error && data) {
        setCta({
          cta_title: data.cta_title || defaultCta.cta_title,
          cta_subtitle: data.cta_subtitle || defaultCta.cta_subtitle,
          cta_button_text: data.cta_button_text || defaultCta.cta_button_text
        })
      }
    }
              ...
              {team.map((member, index) => (
                ...
                >
                  <div className="relative h-64 overflow-hidden bg-gradient-to-br from-brand-sky/20 to-blue-600/20">
                    <img
                      src={member.image_url || '/team-placeholder.jpeg'}
                      alt={member.name}
